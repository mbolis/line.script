 ...
 * @constructor
 */
export const Interpreter = function (code, opt_initFunc) {
 ...
 * @param {boolean=} varArgs Enable var-args mode (callback as first arg)
 * @returns {!Interpreter.Object} New function.
 */
Interpreter.prototype.createAsyncFunction = function (asyncFunc, varArgs) {
...
Interpreter.prototype.setAsyncFunctionPrototype =
  function (obj, name, wrapper, varArgs) {
    this.setProperty(obj.properties['prototype'], name,
      this.createAsyncFunction(wrapper, varArgs),
      Interpreter.NONENUMERABLE_DESCRIPTOR);
  };
  ...
  if (!state.doneExec_) {
    ...
    if (funcNode) {
      ...
      if (!func.varArgs) {
        ...
        // Insert the callback at the beginning
        var argsWithCallback = [callback].concat(state.arguments_);
      }
      this.paused_ = true;
      if (!state.scope.strict) {
        state.funcThis_ = this.boxThis_(state.funcThis_);
